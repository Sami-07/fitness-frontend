    ...
    />
      <div>
        ...
        <div className='flex justify-between pt-4 pb-2'>
          <p>Dinner</p><p>{calories.toFixed(2)} of 712 Cal</p>
        </div>
          ...
          {(result && result.length > 0 && result[0].dinner) && Object.keys(result[0].dinner).map(foodItem => {
            return (
              <div className=' flex justify-between'>
                ...
                <div className='flex gap-4 items-center'><p>{result[0].dinner[foodItem].calories.toFixed(2)} Cal</p><FaCircleMinus  onClick={() => handleDelete(foodItem, "dinner")}  className=' text-xl' /> </div>
              </div>
            )
          })}
...
{searchResults.length > 0 && <div className=' w-full px-2 py-2'>
  ...
  {searchResults.map(item => {
    return (
      ...
      <div key={item} onClick={() => redirectToNutrientsPage({ foodName: item, mealType: "dinner" })} className='cursor-pointer flex justify-between items-center p-4 border-2 mb-1 bg-blue-100 border-gray-400 rounded-xl'>{item} <FiArrowUpRight className='text-2xl' /></div>
